...
Semantic segmentation is a fundamental computer vision task that assigns a class label to every pixel in an image. This dense prediction problem is critical for applications such as autonomous driving, medical image analysis, and scene understanding. The Cityscapes dataset, containing 5,000 high-resolution urban street scene images (1024×2048 pixels) with 19 semantic classes, serves as a benchmark for evaluating semantic segmentation models.

### Current Approaches and Limitations

Traditional approaches to semantic segmentation can be categorized into:

1. **Fully Convolutional Networks (FCNs)**: Process every pixel independently, leading to high computational costs
2. **DeepLab/Atrous Convolution**: Use dilated convolutions to capture multi-scale context but still operate at pixel level
3. **Transformer-based Methods**: Recent approaches like SegFormer and Mask2Former achieve state-of-the-art results but require massive computational resources

**Key Limitations:**

- **Computational Inefficiency**: Processing 2+ million pixels per image (1024×2048) is computationally expensive
- **Memory Requirements**: High-resolution feature maps consume significant GPU memory
- **Redundancy**: Adjacent pixels often belong to the same semantic region, leading to redundant computations

### Our Approach: Superpixel-Based Segmentation

We propose a **decoupled superpixel-based architecture** that addresses these limitations by:

1. **Grouping perceptually similar pixels** into superpixels using SLIC (Simple Linear Iterative Clustering)
2. **Reducing computational complexity** from ~2M pixels to ~2K superpixels (~1000× reduction)
3. **Leveraging Vision Transformers (ViT)** for global context modeling on the reduced token set
4. **Maintaining high accuracy** through multi-scale hypercolumn features from pretrained ResNet-50

This approach combines the efficiency of superpixel grouping with the representation power of deep learning, making semantic segmentation more practical for real-world deployment.
...
- Achieve >50% mIoU on Cityscapes validation set
...
**Architecture: ResNet-50 Backbone with Hypercolumn Features**
...
<!-- **Parameters**: ~23M (frozen) + ~500K (trainable projection layers) --> check how many parameters needed
...
**Architecture: Vision Transformer (ViT) Encoder**
...
1. Multi-scale feature extraction with ResNet-50 backbone
2. GPU-accelerated SLIC superpixel generation
3. Superpixel tokenization and feature pooling
4. Vision Transformer classifier
5. Dense prediction via superpixel association
6. End-to-end training pipeline with Cityscapes dataset
7. Comprehensive evaluation metrics and visualization
...
| Metric | Value | Notes |
|--------|-------|-------|
| **Mean IoU (mIoU)** | **53.77%** | Primary metric for semantic segmentation |
| **Pixel Accuracy** | **90.74%** | Percentage of correctly classified pixels |
| **Test Loss** | **0.3099** | Cross-entropy loss on validation set |
| **Latency** | **54.82 ms** | Per image |

#### Per-Class Performance

The table below shows IoU and accuracy for each of the 19 Cityscapes classes:

| Class | IoU | Accuracy | Support (pixels) |
|-------|-----|----------|------------------|
| **road** | **94.91%** | 97.47% | 345,264,442 |
| **sky** | **89.32%** | 97.35% | 30,765,347 |
| **building** | **84.19%** | 93.12% | 201,005,428 |
| **vegetation** | **84.38%** | 92.35% | 158,868,008 |
| **car** | **83.63%** | 92.55% | 59,731,217 |
| sidewalk | 64.68% | 78.46% | 49,568,652 |
| person | 54.24% | 73.43% | 11,913,424 |
| bus | 54.62% | 67.14% | 3,563,120 |
| bicycle | 52.01% | 63.41% | 6,504,475 |
| traffic sign | 49.13% | 57.56% | 6,098,373 |
| terrain | 48.48% | 70.64% | 7,625,026 |
| train | 48.09% | 56.63% | 1,031,648 |
| truck | 47.18% | 59.87% | 2,760,211 |
| fence | 34.92% | 45.44% | 7,521,741 |
| traffic light | 33.66% | 41.64% | 1,808,393 |
| wall | 27.11% | 28.82% | 6,718,315 |
| pole | 24.35% | 30.36% | 13,565,658 |
| rider | 24.01% | 35.39% | 1,975,596 |
| motorcycle | 22.68% | 29.62% | 729,415 |
...
![Cross Entropy Loss vs Epoch](../assets/cross_entropy_loss.jpeg)

#### Image and Superpixel Segmentation
![Superpixel Generation Output](../assets/image_and_superpixel.jpg)

#### Segmentation Output vs Ground Truth
...
**Immediate Goals (Before Final Report):**

1. **Complete Training**:
   - Train for more epochs and do more finetuning of hyperparameters

2. **Comprehensive Evaluation**:
   - Compute final mIoU, pixel accuracy, and per-class IoU
...
3. **Visualizations**:
   ...
   - Generate qualitative segmentation examples (success and failure cases)
   - Visualize superpixel boundaries and attention maps
...
2. **End-to-End Learning**: Make SLIC differentiable using soft clustering
...
| Name | Contributions |
|------|---------------|
| **Abhishek** | • Implemented ResNet-50 feature extractor with hypercolumn fusion • Setup training pipeline with AdamW optimizer and LR scheduling • Wrote Methods section (Feature Extraction) |
...
| **Kanhaiya** | • Implemented frozen backbone optimization (35-40% speedup) • Fixed optimizer checkpoint loading for frozen backbone • Contributed to Methods section and Results Evaluation • Contributed to Data Pre-Processing |
| **Rishit** | • Implemented and optimized GPU-based confusion matrix computation (40× speedup) • Conducted extensive performance profiling and optimization • Led Results Evaluation and Analysis • Wrote Potential Results & Discussion section |
